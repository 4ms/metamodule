...
public:
	...
	struct i2cTimingReg {
		...
		uint8_t SCLDEL_SDADEL; // top 4 bits: SCLDEL * tPRESC = SCL Delay between SDA edge and SCL
							   // rising edge bottom 4 bits: = SDADEL * tPRESC = SDA Delay between
							   // SCL falling edge and SDA edge
	...
	Error mem_read(uint16_t dev_address,
				   uint16_t mem_address,
				   uint32_t memadd_size,
				   uint8_t *data,
				   uint16_t size);
	Error mem_write(uint16_t dev_address,
					uint16_t mem_address,
					uint32_t memadd_size,
					uint8_t *data,
					uint16_t size);
	Error mem_write_IT(uint16_t dev_address,
					   uint16_t mem_address,
					   uint32_t memadd_size,
					   uint8_t *data,
					   uint16_t size);
	Error mem_write_dma(uint16_t dev_address,
						uint16_t mem_address,
						uint32_t memadd_size,
						uint8_t *data,
						uint16_t size);
