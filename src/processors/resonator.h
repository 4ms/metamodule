...
public:
	virtual float update(float input)
	{
		...
		// bufa[2] = (c0_a * bufa[1] + c1 * bufa[0]) - c2_a * input;
		// bufa[0] = bufa[0] - (c1 * bufa[2]);

		// bufa[1] = bufa[2];
		// float out_a =  bufa[1];

		// buf[2] = (c0 * buf[1] + c1 * buf[0]) - c2  * out_a;
	...
	virtual void set_param(int param_id, float val)
	{
		...
		if (param_id == 0) {
			...
			freq = map_value(val, 1.f, 32.f, minFreq, maxFreq);
		}
		else if (param_id == 1) {
			res = val;
		}
	...
	void calc_coefs() {
		...
		coefs[0].c0 = 1.003225f - expTable.closest(1.0f - res0) / 310.f; // * 96000/samplerate ??
		//coefs[0].c0 = 1.0f - 2.f/(samplerate * expTable.closest(res));
		...
		coefs[0].c2 = (0.003f * coefs[0].c1) - (0.1f * coefs[0].c0) + 0.102f;

		coefs[1].c2 = (0.003f * coefs[1].c1) - (0.1f * coefs[1].c0) + 0.102f;
