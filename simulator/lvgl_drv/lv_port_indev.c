...
static lv_indev_drv_t indev_drv_keyboard_encoder;
...
static keyboard_map_t lvgl_keyboard_rotary_map[] = {
	{.sdl_map = SDLK_ESCAPE, .lvgl_map = LV_KEY_ESC},
	{.sdl_map = SDLK_UP, .lvgl_map = LV_KEY_HOME},	  //Back button
	{.sdl_map = SDLK_DOWN, .lvgl_map = LV_KEY_ENTER}, //Rotary button
	{.sdl_map = SDLK_LEFT, .lvgl_map = LV_KEY_PREV},
	{.sdl_map = SDLK_RIGHT, .lvgl_map = LV_KEY_NEXT},
};
...
static void keyboard_rotary_read(lv_indev_drv_t *indev_drv_keyboard_encoder, lv_indev_data_t *data) {
	static SDL_Event e;
	if (SDL_PollEvent(&e)) {
		if (e.type == SDL_KEYDOWN || e.type == SDL_KEYUP) {

			for (int i = 0; i < (sizeof(lvgl_keyboard_rotary_map) / sizeof(keyboard_map_t)); i++) {
				if (lvgl_keyboard_rotary_map[i].sdl_map == e.key.keysym.sym) {

					switch (lvgl_keyboard_rotary_map[i].lvgl_map) {
						case LV_KEY_ESC:
							//quit?
							break;
						case LV_KEY_HOME:
...
void lv_port_indev_init(bool use_mouse_cursor, bool use_keypad, bool use_keyboard_rotary) {
	...
	if (use_keyboard_rotary) {
		lv_indev_drv_init(&indev_drv_keyboard_encoder);
		indev_drv_keyboard_encoder.type = LV_INDEV_TYPE_ENCODER;
		indev_drv_keyboard_encoder.read_cb = keyboard_rotary_read;
		indev_keypad = lv_indev_drv_register(&indev_drv_keyboard_encoder);
	}
