...
struct LowpassgateModule : CommModule {
	...
	enum ParamIds { LEVEL_PARAM, CV_PARAM, DECAY_PARAM, NUM_PARAMS };
	enum InputIds { CV_INPUT, SIGNAL_INPUT, PING_INPUT, NUM_INPUTS };
	...
	LowpassgateModule()
	{
		...
		inputJacks[LowpassgateModule::PING_INPUT]->scale = [](float f) { return f / 5.0f; };
...
struct LowpassgateWidget : CommModuleWidget {
	...
	LowpassgateWidget(LowpassgateModule *module)
	{
		...
		addLabeledInput("PING", LowpassgateModule::PING_INPUT, {0, 3});
