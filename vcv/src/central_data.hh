...
public:
  ...
  void unregisterModule(ModuleID mod) {
    ...
    auto module_it =
        std::find_if(moduleData.begin(), moduleData.end(), [&mod](auto &m) {
          return m.id == mod.id && m.slug == mod.slug;
        });
    if (module_it != moduleData.end())
      moduleData.erase(module_it);

    std::erase_if(paramData,
                  [=](const auto &p) { return (p.moduleID == mod.id); });
    std::erase_if(jackData, [&](const auto &j) {
      return j.receivedModuleId == mod.id || j.sendingModuleId == mod.id;
    });
    std::erase_if(maps, [&](const auto &m) {
      return m.dst.moduleID == mod.id || m.src.moduleID == mod.id;
    });
  }
  ...
  rack::Module *getRegisteredModulePtr(int64_t moduleID) {
    auto module_it =
        std::find_if(moduleData.begin(), moduleData.end(),
                     [&moduleID](auto &m) { return m.id == moduleID; });
  ...
  bool isRegisteredHub(int64_t moduleId) {
    auto module_it =
        std::find_if(moduleData.begin(), moduleData.end(), [=](auto &m) {
          if (m.id == moduleId) {
  ...
  void registerMapDest(LabelButtonID dest) {
    ...
    { // start mapsmtx lock
      ...
      if (!_currentMap.dst_module) {
        printf(
            "Dest module ptr is null (not registered?). Aborting mapping.\n");
        return;
      }
      ...
      if (!found) {
        ...
        if (_currentMap.src.objType == LabelButtonID::Types::OutputJack) {
          auto num_erased = std::erase_if(
              maps, [&](auto const &m) { return m.src == _currentMap.src; });
          pr_dbg("Removed %lu mappings from centralData, with src on hub: "
                 "m=%lld out-jack=%lld\n",
                 num_erased, _currentMap.src.moduleID, _currentMap.src.objID);
          (void)num_erased;
        }
  ...
  void unregisterKnobMapsBySrcModule(int64_t moduleId) {
    // Remove CD::maps
    {
      std::lock_guard mguard{mtx};
      std::erase_if(maps, [=](const auto &m) {
        return (m.src.objType == LabelButtonID::Types::Knob &&
                m.src.moduleID == moduleId);
      });
    }
  }
  ...
  void setMapRange(LabelButtonID src, LabelButtonID dst, float rmin,
                   float rmax) {
    std::lock_guard mguard{mapsmtx};
    auto m = std::find_if(maps.begin(), maps.end(), [&](const auto &m) {
  ...
  void setMapRangeMin(LabelButtonID dst, float rmin) {
    std::lock_guard mguard{mapsmtx};
    auto m = std::find_if(maps.begin(), maps.end(),
                          [&](const auto &m) { return (m.dst == dst); });
    if (m == maps.end())
      return;
    m->range_min = MathTools::constrain(rmin, 0.f, 1.f);
  }
  ...
  void setMapAliasName(LabelButtonID src, LabelButtonID dst,
                       std::string newname) {
    std::lock_guard mguard{mapsmtx};
    auto m = std::find_if(maps.begin(), maps.end(), [&](const auto &m) {
      return (m.src == src && m.dst == dst);
  ...
  bool isLabelButtonMapped(LabelButtonID const &b) {
    return maps.end() !=
           std::find_if(maps.begin(), maps.end(), [&](const auto &m) {
             return (m.src == b || m.dst == b);
           });
  }
  ...
  unsigned getNumMappingsFromSrc(LabelButtonID const &src) {
    return std::count_if(maps.begin(), maps.end(),
                         [&](const auto &m) { return m.src == src; });
  }
...
private:
  ...
  static inline const std::array<ModuleTypeSlug, 2> ValidHubSlugs = {
      "PANEL_8", "PanelMedium"};
};
