...
class MetaModuleTextBox : public rack::LedDisplayTextField {
	void draw(const DrawArgs &args) override { rack::LedDisplayTextField::draw(args); }
};

struct Davies1900hBlackKnob4ms : MappableKnob<rack::Davies1900hKnob> {
...
struct Davies1900hBlackKnobUnlined4ms : MappableKnob<rack::Davies1900hKnob> {
...
struct DaviesLarge4ms : MappableKnob<rack::Davies1900hKnob> {
	DaviesLarge4ms() { setSvg(APP->window->loadSvg(rack::asset::plugin(pluginInstance, "res/components/Davies_large.svg"))); }
};

struct Small9mmKnob : MappableKnob<rack::RoundKnob> {
	Small9mmKnob() { setSvg(APP->window->loadSvg(rack::asset::plugin(pluginInstance, "res/components/9mm_knob.svg"))); }
};

struct Small9mmUnlinedKnob : MappableKnob<rack::RoundKnob> {
	Small9mmUnlinedKnob()
	{
...
struct FourmsSliderHorizontalLight : rack::RectangleLight<rack::TSvgLight<TBase>> {
	...
	{
		this->setSvg(rack::Svg::load(rack::asset::plugin(pluginInstance, "res/components/FourmsSliderLightHorizontal.svg")));
	}
};
...
struct FourmsSliderHorizontal : rack::app::SvgSlider {
	FourmsSliderHorizontal()
	{
		...
		maxHandlePos = rack::mm2px(rack::math::Vec(22.078, 0.738));
		minHandlePos = rack::mm2px(rack::math::Vec(0.738, 0.738));
		setBackgroundSvg(rack::Svg::load(rack::asset::plugin(pluginInstance, "res/components/FourmsSliderHorizontal.svg")));
	}
};

template<typename TLightBase = rack::RedLight>
struct FourmsLightSliderHorizontal : rack::LightSlider<FourmsSliderHorizontal, FourmsSliderHorizontalLight<TLightBase>> {
	FourmsLightSliderHorizontal()
	{
		// TODO Fix positions
		this->light->box.size = rack::mm2px(rack::math::Vec(3.276, 1.524));
		// TODO Fix SVG
		this->setHandleSvg(rack::Svg::load(rack::asset::plugin(pluginInstance, "res/components/FourmsSliderHorizontalHandle.svg")));
	}
};
