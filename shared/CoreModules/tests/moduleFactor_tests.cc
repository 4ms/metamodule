...
struct TestInfo : ModuleInfoBase {
	...
	static constexpr std::array<KnobDef, 3> Knobs = {{
		{
			.id = 1,
...
constexpr ModuleInfo testinfo{
	.slug = "mno", .width_hp = 4, .NumKnobs = TestInfo::Knobs.size(), .Knobs = TestInfo::Knobs.data(), // works!
																									   // .slug = "mno",
																									   // .width_hp = 4,
																									   // .NumKnobs = 3,
...
TEST_CASE("Register derived object uses its values, not ModuleInfoBase") {
	...
	CHECK(ModuleFactory::getModuleInfo("DEF").slug == "mno");
