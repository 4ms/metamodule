...
private:
	void sideloadDrivers()
	{
		...
		auto pwmToFloat = [](uint16_t pwm_val){return float(pwm_val)/float(4095);};

		setLED<EnvOutLight>(std::array<float,3>{pwmToFloat(peg.pwm_vals[PWM_ENVA_R]),pwmToFloat(peg.pwm_vals[PWM_ENVA_G]),pwmToFloat(peg.pwm_vals[PWM_ENVA_B])});
		setLED<_5VEnvLight>(std::array<float,3>{pwmToFloat(peg.pwm_vals[PWM_ENVB_R]),pwmToFloat(peg.pwm_vals[PWM_ENVB_G]),pwmToFloat(peg.pwm_vals[PWM_ENVB_B])});
		setLED<CycleButton>(std::array<float,3>{pwmToFloat(peg.pwm_vals[PWM_CYCLEBUT_R]),pwmToFloat(peg.pwm_vals[PWM_CYCLEBUT_G]),pwmToFloat(peg.pwm_vals[PWM_CYCLEBUT_B])});
		setLED<PingButton>(std::array<float,3>{pwmToFloat(peg.pwm_vals[PWM_PINGBUT_R]),pwmToFloat(peg.pwm_vals[PWM_PINGBUT_G]),pwmToFloat(peg.pwm_vals[PWM_PINGBUT_B])});
		setLED<EofLight>(pwmToFloat(peg.pwm_vals[PWM_EOF_LED]));
	}
