...
#define LE64(addr)                                                                                                     \
	(((uint64_t)(addr)[0]) | (((uint64_t)(addr)[1]) << 8) | (((uint64_t)(addr)[2]) << 16) |                            \
	 (((uint64_t)(addr)[3]) << 24) | (((uint64_t)(addr)[4]) << 32) | (((uint64_t)(addr)[5]) << 40) |                   \
...
#define USBH_MAX_PIPES_NBR 15U /* mori set to 15, was 16u */
...
#define USBH_CONFIGURATION_DESCRIPTOR_SIZE                                                                             \
	(USB_CONFIGURATION_DESC_SIZE + USB_INTERFACE_DESC_SIZE + (USBH_MAX_NUM_ENDPOINTS * USB_ENDPOINT_DESC_SIZE))
...
typedef struct {
	// TODO: Align these both to 4k?
	uint8_t CfgDesc_Raw[USBH_MAX_SIZE_CONFIGURATION];
	uint8_t Data[USBH_MAX_DATA_BUFFER];
	// Removed when adding hub support (is in mori):
	// These are moved to USBH_TargetTypeDef dev_address and speed
...
typedef struct _USBH_HandleTypeDef {
	...
	__IO HOST_StateTypeDef gPushState; /*  Host State Machine Value to go to after delay*/
	uint32_t tickstart;
	uint32_t wait;
