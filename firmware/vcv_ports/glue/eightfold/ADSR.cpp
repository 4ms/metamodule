	...
	void process(const ProcessArgs &args) override {
		...
		if (cvDivider.process()) {
			...
			for (int c = 0; c < channels; c += 4) {
				// CV
				float_4 attack =
					attackParam + inputs[ATTACK_INPUT].getPolyVoltageSimd<float_4>(c) / 10.f * attackCvParam;
				float_4 decay = decayParam + inputs[DECAY_INPUT].getPolyVoltageSimd<float_4>(c) / 10.f * decayCvParam;
				float_4 sustain =
					sustainParam + inputs[SUSTAIN_INPUT].getPolyVoltageSimd<float_4>(c) / 10.f * sustainCvParam;
				float_4 release =
					releaseParam + inputs[RELEASE_INPUT].getPolyVoltageSimd<float_4>(c) / 10.f * releaseCvParam;
		...
		for (int c = 0; c < channels; c += 4) {
			...
			float_4 lambda = simd::ifelse(attacking[c / 4],
										  attackLambda[c / 4],
										  simd::ifelse(gate[c / 4], decayLambda[c / 4], releaseLambda[c / 4]));
...
struct ADSRWidget : ModuleWidget {
	ADSRWidget(ADSR2 *module) {
		...
		addParam(createLightParamCentered<VCVLightSlider<YellowLight>>(
			mm2px(Vec(6.604, 55.454)), module, ADSR2::ATTACK_PARAM, ADSR2::ATTACK_LIGHT));
		addParam(createLightParamCentered<VCVLightSlider<YellowLight>>(
			mm2px(Vec(17.441, 55.454)), module, ADSR2::DECAY_PARAM, ADSR2::DECAY_LIGHT));
		addParam(createLightParamCentered<VCVLightSlider<YellowLight>>(
			mm2px(Vec(28.279, 55.454)), module, ADSR2::SUSTAIN_PARAM, ADSR2::SUSTAIN_LIGHT));
		addParam(createLightParamCentered<VCVLightSlider<YellowLight>>(
			mm2px(Vec(39.116, 55.454)), module, ADSR2::RELEASE_PARAM, ADSR2::RELEASE_LIGHT));
		addParam(createParamCentered<Trimpot>(mm2px(Vec(6.604, 80.603)), module, ADSR2::ATTACK_CV_PARAM));
		addParam(createParamCentered<Trimpot>(mm2px(Vec(17.441, 80.63)), module, ADSR2::DECAY_CV_PARAM));
		addParam(createParamCentered<Trimpot>(mm2px(Vec(28.279, 80.603)), module, ADSR2::SUSTAIN_CV_PARAM));
		addParam(createParamCentered<Trimpot>(mm2px(Vec(39.119, 80.603)), module, ADSR2::RELEASE_CV_PARAM));
		addParam(createLightParamCentered<VCVLightBezel<WhiteLight>>(
			mm2px(Vec(6.604, 113.115)), module, ADSR2::PUSH_PARAM, ADSR2::PUSH_LIGHT));

		addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(6.604, 96.882)), module, ADSR2::ATTACK_INPUT));
		addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(17.441, 96.859)), module, ADSR2::DECAY_INPUT));
		addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(28.279, 96.886)), module, ADSR2::SUSTAIN_INPUT));
		addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(39.119, 96.89)), module, ADSR2::RELEASE_INPUT));
		addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(17.441, 113.115)), module, ADSR2::GATE_INPUT));
		addInput(createInputCentered<ThemedPJ301MPort>(mm2px(Vec(28.279, 113.115)), module, ADSR2::RETRIG_INPUT));

		addOutput(createOutputCentered<ThemedPJ301MPort>(mm2px(Vec(39.119, 113.115)), module, ADSR2::ENVELOPE_OUTPUT));
...
Model *modelADSR2 = createModel<ADSR2, ADSRWidget>("ADSR2");
