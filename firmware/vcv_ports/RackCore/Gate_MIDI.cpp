...
struct Gate_MIDI : Module {
	...
	enum InputIds { ENUMS(GATE_INPUTS, 16), NUM_INPUTS };
	...
	void process(const ProcessArgs &args) override {
		...
		for (int id = 0; id < 16; id++) {
			...
			if (velocityMode) {
				uint8_t vel =
					(uint8_t)clamp(std::round(inputs[GATE_INPUTS + id].getVoltage() / 10.f * 127), 0.f, 127.f);
...
struct Gate_MIDIWidget : ModuleWidget {
	Gate_MIDIWidget(Gate_MIDI *module) {
		...
		addInput(
			createInputCentered<ThemedPJ301MPort>(mm2px(Vec(19.739, 101.466)), module, Gate_MIDI::GATE_INPUTS + 9));
		addInput(
			createInputCentered<ThemedPJ301MPort>(mm2px(Vec(31.289, 101.466)), module, Gate_MIDI::GATE_INPUTS + 10));
		addInput(
			createInputCentered<ThemedPJ301MPort>(mm2px(Vec(42.838, 101.466)), module, Gate_MIDI::GATE_INPUTS + 11));
		addInput(
			createInputCentered<ThemedPJ301MPort>(mm2px(Vec(8.189, 112.998)), module, Gate_MIDI::GATE_INPUTS + 12));
		addInput(
			createInputCentered<ThemedPJ301MPort>(mm2px(Vec(19.739, 112.984)), module, Gate_MIDI::GATE_INPUTS + 13));
		addInput(
			createInputCentered<ThemedPJ301MPort>(mm2px(Vec(31.289, 112.984)), module, Gate_MIDI::GATE_INPUTS + 14));
		addInput(
