...
private:
	...
	void arc_change_value() {
		...
		float normalized_value = (float)value / range;
		...
		std::visit([&](const auto& el) {
			...
			if constexpr (std::is_base_of_v<Pot, T>) {
				...
				if (el.integral && num_steps > 1) {
					float step_size = 1.0f / (num_steps - 1);
					
					float current_param_value = playloader.param_value(drawn_el->gui_element.module_idx, drawn_el->gui_element.idx.param_idx);
					float movement_direction = normalized_value - current_param_value;
					...
					int snapped_value = std::round(target_step * step_size * range) + lv_arc_get_min_value(ui_ControlArc);
