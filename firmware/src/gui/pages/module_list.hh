...
private:
	...
	static void scroll_cb(lv_event_t *event) {
		...
		if (!page)
			return;
		ModuleTypeSlug selected;
		lv_roller_get_selected_str(event->target, selected._data, selected.capacity);

		ModuleDrawer drawer{page->ui_ModuleListImage, 240};
		auto module_canvas = drawer.draw_faceplate(selected, page->buffer);
		lv_obj_refr_size(module_canvas);
		auto width_px = lv_obj_get_width(module_canvas);
		lv_obj_set_width(page->ui_ModuleListImage, width_px);
		lv_obj_refr_size(page->ui_ModuleListImage);
		drawer.draw_elements(selected, module_canvas);
	}
