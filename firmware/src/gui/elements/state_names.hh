...
inline std::string pot_display_value_string(Pot const &el, float value, bool integral = false) {
	std::string s;
		...
		overloaded{
			[value = value, res = resolution, &s](Pot const &el) {
				if (!has_custom_display(el) && el.min_value == 0 && el.max_value == 1)
					...
					s = pot_display_value_string(el, value);
			},

			[value = value, &s](KnobSnapped const &el) {
				if (el.num_pos > 0 && !has_custom_display(el))
					s = snapped_value_string(el, value);
				else
					s = pot_display_value_string(el, value, true);
			},
