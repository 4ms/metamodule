...
namespace MetaModule::DrawHelper
{
void draw_module(lv_obj_t *canvas, const ModuleInfoView &info, uint32_t zoom) {
	static lv_draw_img_dsc_t draw_img_dsc;
	lv_draw_img_dsc_init(&draw_img_dsc);
	draw_img_dsc.zoom = zoom;
	float adj = (float)(zoom) / 256.f;

	auto scale_and_center = [adj](float x, int element_size) {
		return ModuleInfoBase::mm_to_px<240>((x - element_size / 2) * adj);
	};

	auto scale_center = [adj](auto el, auto img_header) {
		auto x = (ModuleInfoBase::mm_to_px<240>(el.x_mm) - img_header.w / 2) * adj;
		auto y = (ModuleInfoBase::mm_to_px<240>(el.y_mm) - img_header.h / 2) * adj;
