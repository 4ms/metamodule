...
static USBH_StatusTypeDef USBH_MIDI_InterfaceInit(USBH_HandleTypeDef *phost, const USBH_TargetTypeDef *target);
...
static USBH_StatusTypeDef USBH_MIDI_InterfaceInit(USBH_HandleTypeDef *phost, const USBH_TargetTypeDef *target) {
	...
	if (interface == 0xFF) /* No Valid Interface */ {
		...
		USBH_OpenPipe(
			phost, MIDI_Handle->OutPipe, MIDI_Handle->OutEp, target, USB_EP_TYPE_BULK, MIDI_Handle->OutEpSize);

		USBH_OpenPipe(phost, MIDI_Handle->InPipe, MIDI_Handle->InEp, target, USB_EP_TYPE_BULK, MIDI_Handle->InEpSize);
	...
	switch (MIDI_Handle->data_tx_state) {
		...
		case MIDI_SEND_DATA:
			if (MIDI_Handle->TxDataLength > MIDI_Handle->OutEpSize) {
				USBH_BulkSendData(phost, MIDI_Handle->pTxData, MIDI_Handle->OutEpSize, MIDI_Handle->OutPipe, 1);
			} else {
				USBH_BulkSendData(phost, MIDI_Handle->pTxData, MIDI_Handle->TxDataLength, MIDI_Handle->OutPipe, 1);
			}
...
static void MIDI_ProcessReception(USBH_HandleTypeDef *phost) {
	...
	switch (MIDI_Handle->data_rx_state) {
		...
		case MIDI_RECEIVE_DATA:
			...
			USBH_BulkReceiveData(phost, MIDI_Handle->pRxData, MIDI_Handle->InEpSize, MIDI_Handle->InPipe);
