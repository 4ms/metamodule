...
struct PluginLoadUntar {
	...
	std::string untar_contents(std::span<uint8_t> buffer, PluginFile &plugin_file) {
		...
		if (fw_version.can_host_version(VersionUtil::Version(plugin_vers))) {
			...
			return "Plugin " + plugin_name + ": version is " + plugin_vers + ", but firmware version is " + fw_vers;
		}
	}
