...
struct PatchPlayLoader {
	...
	void request_new_audio_settings(uint32_t sample_rate, uint16_t block_size, uint32_t max_retries) {
		pr_dbg("PatchPlayLoader::request_new_audio_settings %u %u %u\n", sample_rate, block_size, max_retries);

		// Debug::Pin1::high();
		// audio_settings_changed_.store(false, std::memory_order_acquire);
		bool should_play = is_playing();
		if (should_play) {
			stop_audio();
			while (!is_audio_muted())
		...
		// uint32_t tm = get_time();
		// while (audio_settings_changed_.load(std::memory_order_acquire) == false) {
		// 	if (get_time() > tm + 2000) {
		// 		pr_err("Audio did not update audio settings\n");
		// 		break;
		// 	}
		// }
		if (should_play) {
	...
	void notify_audio_settings_activated() {
		// audio_settings_changed_.store(true, std::memory_order_release);
	}
...
private:
	...
	std::atomic<bool> audio_settings_changed_ = false;
