...
void update_all_roto_controls(PatchData &pd, std::span<std::unique_ptr<CoreProcessor>> modules) {
	...
	for (const auto &k : pd.midi_maps.set) {
		if (!(k.is_midi_cc() || k.is_midi_notegate())) {
			// Only configure RotoControl for valid MIDI CC or NoteGate types handled by original logic
			pr_warn(
				"RotoControl: Skipping MIDI map for module_id %u (num_modules: %u, slug_count: %zu, module_ptr: %p)\n",
				k.module_id,
				num_modules,
				pd.module_slugs.size(),
				(k.module_id < modules.size() ? modules[k.module_id].get() : nullptr));
			continue;
		}

		pr_dbg("RotoControl: Processing MIDI map for module_id %u (num_modules: %u, slug_count: %zu, module_ptr: %p)\n",
			   k.module_id,
			   num_modules,
			   pd.module_slugs.size(),
			   (k.module_id < modules.size() ? modules[k.module_id].get() : nullptr));

		if (k.module_id > 0 && k.module_id < pd.module_slugs.size() && k.module_id < num_modules &&
			modules[k.module_id])
		{
							...
							if constexpr (std::is_base_of_v<ParamElement, T>) {
								...
								if constexpr (std::is_base_of_v<Knob, T>) {
												  ...
												  std::is_same_v<T, MomentaryButtonRGB>)
									{
										...
										pr_warn(
											"RotoControl: Encountered an unhandled Button-derived type for param_id %u",
											k.param_id);
									}
								}
								...
								if (haptic_steps == 2) { // This condition was for calling set_switch_control_config
									RotoControl::set_control_config(
										ControlType::SWITCH,
										0,							  // setup_index
									...
									pr_dbg(
										"Setting knob control config, setup_index: %d, control_index: %d, haptic_mode: "
			...
			// Optional: Log if module_id is out of bounds or module not loaded for a MIDI map entry.
			if (k.is_midi_cc() || k.is_midi_notegate()) { // Only warn for types we'd otherwise process
				pr_warn("RotoControl: Skipping MIDI map for module_id %u (num_modules: %u, slug_count: %zu, "
						"module_ptr: %p)\n",
						k.module_id,
						num_modules,
						pd.module_slugs.size(),
