...
#include "VCV_adaptor/simd/Vector.hpp"
#include "VCV_adaptor/simd/functions.hpp"
...
inline T blackman(T alpha, T p) {
	return +(1 - alpha) / 2 - T(1) / 2 * simd::cos(2 * T(M_PI) * p) + alpha / 2 * simd::cos(4 * T(M_PI) * p);
}
...
inline T blackmanNuttall(T p) {
	return +T(0.3635819) - T(0.4891775) * simd::cos(2 * T(M_PI) * p) + T(0.1365995) * simd::cos(4 * T(M_PI) * p) -
...
inline T blackmanHarris(T p) {
	return +T(0.35875) - T(0.48829) * simd::cos(2 * T(M_PI) * p) + T(0.14128) * simd::cos(4 * T(M_PI) * p) -
