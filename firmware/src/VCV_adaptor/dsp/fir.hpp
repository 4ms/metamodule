#pragma once
#include "VCV_adaptor/dsp/common.hpp"
...
struct RealTimeConvolver {
	...
	float *kernelFfts = nullptr;
	float *inputFfts = nullptr;
	float *outputTail = nullptr;
	float *tmpBlock = nullptr;
	size_t blockSize;
	...
	void setKernel(const float *kernel, size_t length) {
		...
		if (kernel && length > 0) {
			...
			for (size_t i = 0; i < kernelBlocks; i++) {
				...
				// Compute fft
				pffft_transform(pffft, tmpBlock, &kernelFfts[blockSize * 2 * i], nullptr, PFFFT_FORWARD);
			}
		}
	}
	...
	*/
	void processBlock(const float *input, float *output) {
		...
		// Compute input fft
		pffft_transform(pffft, tmpBlock, &inputFfts[blockSize * 2 * inputPos], nullptr, PFFFT_FORWARD);
		// Create output fft
		std::memset(tmpBlock, 0, sizeof(float) * blockSize * 2);
		...
		for (size_t i = 0; i < kernelBlocks; i++) {
			...
			pffft_zconvolve_accumulate(
				pffft, &kernelFfts[blockSize * 2 * i], &inputFfts[blockSize * 2 * pos], tmpBlock, 1.f);
		}
		// Compute output
		pffft_transform(pffft, tmpBlock, tmpBlock, nullptr, PFFFT_BACKWARD);
